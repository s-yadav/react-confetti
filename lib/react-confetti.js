...
function confetti(canvasObj) {
  ...
  function self() {
    ...
    ParticleGenerator.prototype.animate = function animateParticle() {
      if (this.particlesGenerated < this.number) {
        var newParticleX = utils.clamp(utils.randomRange(this.x, canvas.width + this.x), this.x, canvas.width + this.x);
      ...
      for (var i = 0; i < this.particles.length; i++) {
        ...
        if (p.y > canvas.height || p.y < -100 || p.x > canvas.width + 100 || p.x < -100) {
          if (this.recycle) {
            ...
            var _newParticleX = utils.clamp(utils.randomRange(this.x, canvas.width + this.x), this.x, canvas.width + this.x);
    ...
    var generator1 = new ParticleGenerator(0, 0, canvas.width, 0, numberOfPieces);
  ...
  self.recycle = function () {
    ...
    recycle = arguments.length <= 0 ? undefined : arguments[0];
    return self;
  };

  self.shapes = function () {
    if (!arguments.length) {
      return shapes;
    }
    shapes = arguments.length <= 0 ? undefined : arguments[0];
    return self;
  };
...
var _confetti = __webpack_require__(5);
...
var Confetti = function (_React$PureComponent) {
  _inherits(Confetti, _React$PureComponent);
  ...
  _createClass(Confetti, [{
    ...
    value: function componentDidMount() {
      this.confetti = (0, _confetti2.default)(this.canvas).numberOfPieces(this.props.numberOfPieces).friction(this.props.friction).wind(this.props.wind).gravity(this.props.gravity).colors(this.props.colors).opacity(this.props.opacity).shapes(this.props.shapes).recycle(this.props.recycle)();
    }
  }, {
    key: 'componentWillReceiveProps',
    value: function componentWillReceiveProps(nextProps) {
      this.confetti.numberOfPieces(nextProps.numberOfPieces).friction(nextProps.friction).wind(nextProps.wind).gravity(nextProps.gravity).colors(nextProps.colors).opacity(nextProps.opacity).shapes(nextProps.shapes).recycle(nextProps.recycle);
    }
  }, {
    ...
    value: function render() {
      ...
      var _props = this.props,
          ...
          passedProps = _objectWithoutProperties(_props, ['width', 'height', 'style', 'numberOfPieces', 'friction', 'wind', 'gravity', 'colors', 'opacity', 'recycle']);
...
Confetti.propTypes = {
  ...
  numberOfPieces: _propTypes2.default.number,
  friction: _propTypes2.default.number,
  wind: _propTypes2.default.number,
  gravity: _propTypes2.default.number,
  colors: _propTypes2.default.arrayOf(_propTypes2.default.string),
  opacity: _propTypes2.default.number,
  recycle: _propTypes2.default.bool,
  shapes: _propTypes2.default.arrayOf(_propTypes2.default.oneOf(['rectangle', 'circle']))
};
